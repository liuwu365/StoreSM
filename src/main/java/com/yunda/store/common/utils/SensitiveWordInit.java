...
public class SensitiveWordInit {
	 ...
	 * @return
	 */
	public static HashMap initKeyWord(List<Map<String, Object>> sensitiveWords) {
		Set<String> keyWordSet = new HashSet<String>();
		try {
			// 从敏感词集合对象中取出敏感词并封装到Set集合中
			for (Map<String, Object> map : sensitiveWords) {
				keyWordSet.add(map.get("word_name").toString());
			}
		}
		catch (Exception e) {
			e.printStackTrace();
		}
		return addSensitiveWordToHashMap(keyWordSet);// 将敏感词库加入到HashMap中
	}
